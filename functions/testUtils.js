...
async function _add_IEX_symbol(name, date) {
  return await _add_symbol(name, date, "iex");
}

add_IEX_symbol = _add_IEX_symbol;

async function _add_FMP_symbol(name, date) {
  return await _add_symbol(name, date, "fmp");
}

add_FMP_symbol = _add_FMP_symbol();

async function _add_symbol(name, date, sourceName) {
...
async function _disable_IEX_symbol(symbol, date) {
  await _disable_symbol(symbol, date, "iex");
}

disable_IEX_symbol = _disable_IEX_symbol;

async function _disable_FMP_symbol(symbol, date) {
  await _disable_symbol(symbol, date, "fmp");
}

disable_FMP_symbol = _disable_FMP_symbol;

async function _disable_symbol(symbol, date, sourceName) {
  symbol.e = false;
  const source = sourceByName[sourceName];
  const collection = source.db.collection('symbols')
...
disable_symbol = _disable_symbol;

async function _enable_IEX_symbol(symbol, date) {
  await _enable_symbol(symbol, date, "iex");
}

enable_IEX_symbol = _enable_IEX_symbol;

async function _enable_FMP_symbol(symbol, date) {
  await _enable_symbol(symbol, date, "fmp");
}

enable_FMP_symbol = _enable_FMP_symbol;

async function _enable_symbol(symbol, date, sourceName) {
  delete symbol.e;
  const source = sourceByName[sourceName];
  const collection = source.db.collection('symbols');
...
enable_symbol = _enable_symbol;

async function _update_IEX_symbol(symbol, date) {
  await _update_symbol(symbol, date, "iex");
}

update_IEX_symbol = _update_IEX_symbol;

async function _update_FMP_symbol(symbol, date) {
  await _update_symbol(symbol, date, "fmp");
}

update_FMP_symbol = _update_FMP_symbol;

async function _update_symbol(symbol, date, sourceName) {
  const source = sourceByName[sourceName];
  const collection = source.db.collection('symbols');
